...
//    private UndoRedo.Manager undoRedoManager;
     ...
     */
    public void addRow() {
        List<String> data = new ArrayList<>(headers.size());
    ...
    private ArrayList<String> getClonedTableRowObject(final ArrayList<String> value) {
        ArrayList<String> cloned = new ArrayList<>(value.size());
        cloned.addAll(value);
        return cloned;
    }
...
//                undoRedoManager.undoableEditHappened(new UndoableEditEvent(this, new CellUndoableEdit(this, oldValue, value, row, column)));
    ...
    public CSVTableModel clone() {
        ...
        if (headers != null) {
            List<String> cloned = new ArrayList<>(headers.size());
            for (String header : headers)
                cloned.add(header);
            model.setHeaders(cloned);
        }
        if (values != null) {
            List<List<String>> cloned = new ArrayList<>(values.size());
            for (List<String> row : values) {
                List<String> clonedRow = new ArrayList<>(row.size());
                for (String s : row)
                    clonedRow.add(s);
